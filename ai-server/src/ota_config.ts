...
function advertisedWsBase(hostHeader: string | undefined): string {
...
export function serveOtaRequest(req: http.IncomingMessage, res: http.ServerResponse): boolean {
    const url = new URL(req.url ?? '/', 'http://127.0.0.1')
    // Accept both /ota and a trailing variant; firmware appends no extra path.
    if (url.pathname !== '/ota' && url.pathname !== '/ota/') return false
